...
export default function StudentPortalPage() {
  ...
  const load = async () => {
    ...
    markAnnouncementsSeen()
    // جلب إعدادات الموقع العامة (اسم المدرس/السنتر) لعرضها في ترويسة البوابة
    try {
      const pub = await fetchPublicSettings()
      for (const [key, value] of Object.entries(pub)) writeSetting(key, value)
    } catch { /* بدون اتصال — يُعرض الاسم الافتراضي */ }
    setMounted(true)
  }
